...
from model import slowly_varying_8, latin_hypercube_sample, boundary_sample, plot_1d
...
with open('groundtruth_generation/slow_varying_eps_1_8.pkl', 'rb') as file:
...
def retrain(model, x_train, x_boundary, u_boundary, x_test, u_test, optimizer, epoch, idx=''):
    ...
    plot_1d(f, x_test, ylim=[-1,3], path="./slowly_varying_8/res_comb"+idx+".png")
    print('Test residual MSE:', (np.mean(np.square(f))))

    rmse, u_pred = model.test(x_test,u_test)
    print('Test RMSE:', rmse)
    plot_1d(u_pred, x_test, u_gt=u_test, ylim=[0,.5], path="./slowly_varying_8/pre_comb"+idx+".png")

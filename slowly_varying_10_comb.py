...
from model import slowly_varying_10, latin_hypercube_sample, boundary_sample, plot_1d
...
with open('groundtruth_generation/slow_varying_eps_1_10.pkl', 'rb') as file:
    u_test = pickle.load(file)
u_test = np.array(u_test).reshape(u_test.shape[0],1)

''' model '''
layers = np.array([9,40,40,40,40,40,40,40,1]) #7 hidden layers
PINN = slowly_varying_10(layers)
PINN.to(PINN.device)
PINN.bc_weight = 200
PINN.fourier = [4]
...
def retrain(model, x_train, x_boundary, u_boundary, x_test, u_test, optimizer, epoch, idx=''):
    ...
    plot_1d(f, x_test, ylim=[-1,3], path="./slowly_varying_10/res_comb"+idx+".png")
    print('Test residual MSE:', (np.mean(np.square(f))))

    rmse, u_pred = model.test(x_test,u_test)
    print('Test RMSE:', rmse)
    plot_1d(u_pred, x_test, u_gt=u_test, ylim=[0,.5], path="./slowly_varying_10/pre_comb"+idx+".png")

...
def retrain(model, x_train, x_boundary, u_boundary, x_test, u_test, optimizer, epoch, idx=''):
    ...
    plot_2d(f.reshape(u_test.shape).T, y, x, path="./diff_reac_50/res_pinn"+idx+".png")
    print('Test residual MSE:', (np.mean(np.square(f))))

    rmse, u_pred = model.test(x_test,u_test.flatten()[:,None])
    print('Test RMSE:', rmse)
    plot_2d(u_pred.reshape(u_test.shape).T, y, x, path="./diff_reac_50/pre_pinn"+idx+".png")
    plot_2d(np.abs(u_pred.reshape(u_test.shape)-u_test).T, y, x, path="./diff_reac_50/diff_pinn"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[200,:], x, u_gt=u_test[200,:], path="./diff_reac_50/sec4_pinn"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[300,:], x, u_gt=u_test[300,:], path="./diff_reac_50/sec6_pinn"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[400,:], x, u_gt=u_test[400,:], path="./diff_reac_50/sec8_pinn"+idx+".png")

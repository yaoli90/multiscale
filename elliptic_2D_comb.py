...
''' model '''
layers = np.array([18,40,40,40,40,40,40,40,40,1]) #7 hidden layers
...
def retrain(model, x_train, x_boundary, u_boundary, x_test, u_test, optimizer, epoch, idx=''):
    ...
    plot_2d(f.reshape(u_test.shape), x, y, path="./elliptic_2D/res_comb"+idx+".png")
    print('Test residual MSE:', (np.mean(np.square(f))))

    rmse, u_pred = model.test(x_test,u_test.flatten()[:,None])
    print('Test RMSE:', rmse)
    plot_2d(u_pred.reshape(u_test.shape), x, y, path="./elliptic_2D/pre_comb"+idx+".png")
    plot_2d(np.abs(u_pred.reshape(u_test.shape)-u_test), x, y, path="./elliptic_2D/diff_comb"+idx+".png")

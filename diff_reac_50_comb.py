...
from model import diff_reac_50, latin_hypercube_sample, boundary_sample, plot_2d, plot_1d
...
file = open('groundtruth_generation/diff_reac_eps_1_50.pkl', 'rb')
u_test = pickle.load(file).T
plot_2d(u_test.T, y, x, path="./diff_reac_50/gt.png")

''' model '''
layers = np.array([10,40,40,40,40,40,40,40,40,1]) #7 hidden layers
PINN = diff_reac_50(layers)
PINN.to(PINN.device)
PINN.bc_weight = 200
PINN.fourier = [4,0]
...
def retrain(model, x_train, x_boundary, u_boundary, x_test, u_test, optimizer, epoch, idx=''):
    ...
    plot_2d(f.reshape(u_test.shape).T, y, x, path="./diff_reac_50/res_comb"+idx+".png")
    print('Test residual MSE:', (np.mean(np.square(f))))

    rmse, u_pred = model.test(x_test,u_test.flatten()[:,None])
    print('Test RMSE:', rmse)
    plot_2d(u_pred.reshape(u_test.shape).T, y, x, path="./diff_reac_50/pre_comb"+idx+".png")
    plot_2d(np.abs(u_pred.reshape(u_test.shape)-u_test).T, y, x, path="./diff_reac_50/diff_comb"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[200,:], x, u_gt=u_test[200,:], path="./diff_reac_50/sec4_comb"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[300,:], x, u_gt=u_test[300,:], path="./diff_reac_50/sec6_comb"+idx+".png")
    plot_1d(u_pred.reshape(u_test.shape)[400,:], x, u_gt=u_test[400,:], path="./diff_reac_50/sec8_comb"+idx+".png")
